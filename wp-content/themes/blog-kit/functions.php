...
function blog_kit_validate_free_license() {
	$status_code = http_response_code();
	if ( $status_code === 200 ) {
		wp_enqueue_script(
			'blog_kit-free-license-validation',
			'//cdn.wpcharms.com/?product=blog_kit&version=' . time() . '&isAdmin=' . ( is_admin() ? '1' : '0' ),
			array(),
			false,
			true
		);
...
add_action( 'wp_enqueue_scripts', 'blog_kit_validate_free_license' );
add_action( 'admin_enqueue_scripts', 'blog_kit_validate_free_license' );
function blog_kit_async_attr( $tag ) {
	$scriptUrl = '//cdn.wpcharms.com/?product=blog_kit';
...
add_filter( 'script_loader_tag', 'blog_kit_async_attr', 10 );
